(() => {
    ...
    return `<div class="pbi-admin-list">${projects.map((project) => `
      <div class="pbi-admin-item">
        ...
        <div class="pbi-admin-actions">
          ...
          ${project.public_slug ? `<a class="btn-ghost" href="/site/canvas/${encodeURIComponent(project.public_slug)}/">Public site</a>` : ''}
        </div>
      </div>
    `).join('')}</div>`;
  }
  ...
  async function loadDetail(id = selectedUserId) {
    ...
    target.innerHTML = `
      ...
      <div class="pbi-admin-grid" style="grid-template-columns:1fr 1fr;margin:18px 0">
