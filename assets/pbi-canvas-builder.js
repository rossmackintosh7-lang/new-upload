(() => {
  ...
  function templateBlocks(name = "local-service") {
    ...
    const base = [
      ...
      block("floatingCard", { title: "Local proof", text: "Freeform badge. Drag this anywhere on desktop.", button: "See proof", background: "#ffffff", accent: p.accent, x: 72, y: 210, width: 310, rotate: -2 }),
    ...
    if (key === "shop") base.splice(8, 0, block("productGrid", { background: p.bg, accent: p.accent, image: p.image }));
    if (key === "cafe") base.splice(10, 0, block("booking", { background: p.bg, accent: p.accent }));
    if (key === "holiday") base.splice(9, 0, block("map", { background: p.bg, accent: p.accent }));
    if (key === "consultant") base.splice(9, 0, block("cmsList", { background: p.soft, accent: p.accent }));
    return base;
  }
