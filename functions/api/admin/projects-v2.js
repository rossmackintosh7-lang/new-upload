...
export async function onRequestGet({ request, env }) {
  ...
  if (projectId) {
    ...
    const assisted_requests = await all(env, `
      SELECT *
      FROM admin_requests
      WHERE project_id = ?
        AND request_type IN ('assisted_build', 'custom_build')
      ORDER BY
        CASE WHEN payment_status = 'paid' THEN 0 ELSE 1 END,
        datetime(created_at) DESC
      LIMIT 20
    `, projectId);
    return json({ ok: true, project: project ? { ...project, data: parseProjectData(project) } : null, sections, notes, assisted_requests });
  }
