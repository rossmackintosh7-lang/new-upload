...
export async function onRequestGet({ request, env }) {
  ...
  const rows = await env.DB.prepare('SELECT id, project_id, filename, content_type, size, url, alt, storage_key, created_at FROM media_assets WHERE project_id = ? AND user_id = ? ORDER BY datetime(created_at) DESC LIMIT 200').bind(projectId, auth.user.id).all();
