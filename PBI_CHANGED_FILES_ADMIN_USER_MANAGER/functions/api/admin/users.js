...
export async function onRequestGet({ request, env }) {
  ...
  if (userId) {
    ...
    return json({ ok: true, user: { ...user, email_verified: Boolean(user.email_verified) }, controls: { notes: user.notes || '', status: user.status || 'active' }, projects, sessions, counts: { canvas: canvas?.count || 0, cms: cms?.count || 0 } });
  }
